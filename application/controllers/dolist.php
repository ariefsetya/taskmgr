...
class Dolist extends CI_Controller {
    ...
    public function get_group(){
        $id = $this->input->post('id');
        $this->cekid($id);
        $db = $this->db->query("select*from menu where idpengguna='$id' and jenis='list' order by judul asc");
    ...
    public function get_artikel(){
        ...
        foreach ($hasil as $key) {
            ...
            if($key['progress']==1){
                ...
                $pro = "<i class='icon-busy' style='color:red'></i> On Progress";
            } 

            $state = "";
                                
            if($key['priority']=="1"){
                $state = "<i class='icon-flag-2' style='color:#0099ee;'></i> Low";
            }
            if($key['priority']=="2"){
                $state = "<i class='icon-flag-2' style='color:orange;'></i> Medium";
            }
            if($key['priority']=="3"){
                $state = "<i class='icon-flag-2' style='color:red;'></i> High";
            }

            $data[$i] = array(($i+1),$state,$key['judul'],$key['tglselesai'],$key['checkby'],$pro,"<span title='Click to View this task' onclick=\"dlglist('".$key['id']."')\"><i class='icon-file fg-blue'></i></span> <span title='Click to Edit this task' onclick='editlistid(".$key['id'].")'><i class='icon-pencil fg-orange'></i></span> <span title='Click to Delete this task' onclick='deltask(".$key['id'].")'><i class='icon-remove fg-red'></i></span>");
            $i++;
        }
