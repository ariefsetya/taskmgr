...
class Files extends CI_Controller {
    ...
    public function get_group(){
        $id = $this->input->post('id');
        $this->cekid($id);
        $db = $this->db->query("select*from menu where idpengguna='$id' and jenis='files' order by judul asc");
    ...
    public function get_artikel(){
        $id = $this->input->post('id');
        $this->cekid($id);
        $db = $this->db->query("select*from berkas where idbookpad='$id' order by nama asc");
        $hasil = $db->result_array();
        ...
        foreach ($hasil as $key) {
            ...
            $embed = str_replace(array('" width="476" height="288" frameborder="0" scrolling="no"></iframe>','<iframe src="'),"",$key['embed']);
            for($a=0;$a<strlen($embed);$a++){
                if(substr($embed, $a,1)=='"'){
                    $embed = substr($embed, 0,$a);
                    goto lanjut;
                }
            }
            lanjut:
            $data[$i] = array(($i+1),$key['nama'],$key['customer'],$key['file_tipe'],$key['source_file'],$key['tempat'],$key['file_version'],"<span title='Click to View this data' onclick=\"openlink('".$embed."')\"><i class='icon-file fg-blue'></i></span> <span title='Click to Edit this data' onclick='editart(".$key['id'].")'><i class='icon-pencil fg-orange'></i></span> <span title='Click to Delete this data' onclick='deleteart(".$key['id'].",".$key['idbookpad'].")'><i class='icon-remove fg-red'></i></span>");
            $i++;
        }
    ...
    public function get_artikelid(){
        $id = $this->input->post('id');
        $this->cekid($id);
        $db = $this->db->query("select*from berkas where id='$id'");
        $hasil = $db->row_array();
        print_r(json_encode($hasil));
    }
    ...
    public function update_files()
    {
        ...
        $new = mysql_query("select*from berkas where id='$id'");
        $data2 = mysql_fetch_array($new);
        print_r(json_encode(array("idbookpad"=>$data2['idbookpad'])));
    ...
    public function save_files()
    {
        ...
        $new = mysql_query("select*from berkas where id='$in_id'");
        $data2 = mysql_fetch_array($new);
